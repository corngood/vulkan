...
writeCommand c builtin = do
  ...
  if not builtin then do
    ...
    tellRequiredName (ExternalName (ModuleName "Graphics.Vulkan.Raw.DeviceInitialization") getProcAddr)
...
convertFunction t@(TyFun (TyCon (UnQual (Ident n))) _)
  | n `elem` ["VkCommandBuffer"] = do
      tellRequiredName (ExternalName (ModuleName "Graphics.Vulkan.Raw.Device") "VkDevice")
