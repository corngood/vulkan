...
typeDeclToVertex :: SpecGraph -> [String] -> TypeDecl -> Vertex
typeDeclToVertex graph tags td =
  ...
  in case td of
       ...
       T.ABitmaskType bmt ->
         let bm = (lookupNameMay =<< ((++ tag) <$> swapSuffix "Flags" "FlagBits" baseName))
         ...
         where (baseName, tag) = breakNameTag tags (bmtName bmt)
       T.AHandleType ht ->
